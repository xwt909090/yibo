            ...
            public void onClick(View v){
                ...
                dialog.setDefault("设为默认", new CarNumDialog.OnDefaultListtener() {
                    ...
                    public void onDefault(CarNumDialog mdialog) {
                        Toast.makeText(v1.getContext(), "default",Toast.LENGTH_SHORT).show();
                    }
                });
                dialog.setDelete("删除", new CarNumDialog.OnDeleteListtener() {
                    @Override
                    public void onDelete(CarNumDialog mdialog) {
                        Toast.makeText(v1.getContext(), "delete", Toast.LENGTH_SHORT).show();
                    }
                });
                dialog.show();
            }
        });
        /*holder.CarNum.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View v){
                int position = holder.getAdapterPosition();
                Car car = mCarsList.get(position);
                Toast.makeText(v.getContext(), "you clicked button " + car.getCarNumber(), Toast.LENGTH_SHORT).show();
            }
        });*/
        return holder;
    }
